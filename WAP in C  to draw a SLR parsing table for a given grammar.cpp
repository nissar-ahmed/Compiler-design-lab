...
void SLR()
{
intclno,no=0,x,y,z,len,cnt=-1,d=0;
closure[i][j].pno=0;
closure[i][j++].dpos=3;
find_closure(no,3);
sclosure(i,j);
...
void sclosure(intclno,intprodno)
{
struct node temp;
  for(inti=0;i<prodno-1;i++)
  {
...
intfind_index(char temp)
{
for(inti=0;i<=tt+tnt;i++)
{
  if(i<=tt)
  {
   if(T[i]==temp)
